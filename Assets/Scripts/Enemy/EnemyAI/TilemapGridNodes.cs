...
public class TilemapGridNodes : MonoBehaviour
{
    public Grid grid;
    ...
    public void CreateNodes()
    {
        ...
        for (int x = searchBounds.startX; x < searchBounds.endX; x++)
        {
            ...
            {
                // Get the tile at the current coordinate in the world 
                TileBase floorTile = floorLayer.GetTile(new Vector3Int(x, y, 0));

                bool obstacleFound = false;
                if (floorTile != null)
                {
                    foreach (Tilemap tilemap in obstacleLayers)
                    {
                        TileBase obstacleTile = tilemap.GetTile(new Vector3Int(x, y, 0));
